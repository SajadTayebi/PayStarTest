...
class Payment
{
    public function createOrder($addressId, $amounts, $token, $gateway_name, $ref_num, $sign)
    {
        try {
            DB::beginTransaction();
    ...
    public function updateOrder($refNum)
    {
        try {
            DB::beginTransaction();

            $transaction = Transaction::where('ref_num', $refNum)->firstOrFail();
            ...
            auth()->user()->notify(new PaymentReceipt($order->id , $order->paying_amount , $refNum));
